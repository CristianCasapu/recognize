 ...
 * by a classifier that could not measure the crop), one batch of files per run, and then
 * weighs the faces of each photo against one another (subject or surroundings, 12.9);
 * re-queues itself until everything is scored. Scheduled by ClusterFacesJob and by the
 * admin "Rescan".
 */
final class FaceQualityJob extends QueuedJob {
	public const FILES_PER_RUN = 150;
	public const SUBJECT_FILES_PER_RUN = 2000;
	...
	protected function run($argument): void {
		try {
			...
			// the scores are in: tell the people the picture is about from the surroundings
			// (nothing is read from disk here, so a much bigger batch fits in one run)
			$subjects = $this->quality->backfillSubjects(self::SUBJECT_FILES_PER_RUN);
			if ($subjects['remaining'] > 0 && $subjects['files'] > 0) {
				$this->jobList->add(self::class, $argument);
			}
		} catch (\Throwable $e) {
	...
	public static function scheduleIfNeeded(IJobList $jobList, FaceQuality $quality): bool {
		if (($quality->countMissing() === 0 && $quality->countMissingSubject() === 0) || $jobList->has(self::class, null)) {
			return false;
		}
